...
const _: _ = (function () {
  ...
  const isFunction: isFunction = function isFunction(value): value is func {
    if (!isObject(value)) {
      return false;
    }
  ...
  function result(object: {}, path: string | string[], defaultValue?: any): any {
  ...
  function baseIsEqual(value, other, bitmask?: any, customizer?: any, stack?: any) {
    if (value === other) {
      return true;
    }
  ...
  function reduce<T extends object, TResult>(collection: T, iteratee: MemoObjectIterator<T[keyof T], T[keyof T], T>, accumulator?: TResult) {
  ...
  function baseClone(value, bitmask, customizer?: Function, key?: any, object?: {}, stack?: any) {
    var result,
...
toArray(): any[];
  ...
  function toArray<T>(value?: List<T> | T | Dictionary<T> | NumericDictionary<T> | null | undefined): T[] | Array<T[keyof T]> | any[] {
    if (!value) {
      return [];
    }
  ...
  function baseFlatten(array, depth, predicate?: any, isStrict?: boolean, result?: any) {
    var index = -1,
      length = array.length;
  ...
  function dropRight<T>(array: ArrayLike<T>, n?: number, guard?: any) {
    var length = array == null ? 0 : array.length;
    if (!length) {
      return [];
    }
  ...
  function debounce(func, wait, options): ((...args: any[]) => any) & Cancelable {
    var lastArgs,
      lastThis,
      maxWait,
      result,
   ...
   */
  function throttle<T extends (...args: any[]) => any>(func: T, wait?: number, options?: ThrottleSettings): any {
    var leading = true,
      trailing = true;
  ...
  type countBy = <T extends object>(
    collection: T | null | undefined,
    iteratee?: ValueIteratee<T[keyof T]>
  ) => Dictionary<number>;
  const countBy: countBy = createAggregator(function (result, value, key) {
    if (hasOwnProperty.call(result, key)) {
      ++result[key];
    } else {
