type renderView = () => void;
export function renderView(view: GenericView) {
  if (view._isRendered) {
    return;
  }
...
type destroyView = () => void;
export function destroyView(view: GenericView, disableDetachEvents: boolean) {
  if (view.destroy) {
